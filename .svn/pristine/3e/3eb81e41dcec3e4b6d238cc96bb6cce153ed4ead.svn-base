...
public class MongoDBDaoImpl {
     ...
     * @param uri
     */
    private MongoDBDaoImpl(String uri) {
        if (mongoClient == null) {
            ...
            build.threadsAllowedToBlockForConnectionMultiplier(300);// 线程队列数，如果连接线程排满了队列就会抛出“Out of semaphores to get db”错误。
     ...
     * @return
     */
    public synchronized static MongoDBDaoImpl getInstance(String uri) {
        if (mongoDBDaoImpl == null) {
            mongoDBDaoImpl = new MongoDBDaoImpl(uri);
        }
    ...
    public boolean replaceOne(String dbName, String collectionName,Document newdocument,
    ...
    public boolean bultWriteRecord(String dbName, String collectionName,List<WriteModel<Document>> requests){
        MongoCollection<Document> collection=mongoClient.getDatabase(dbName).getCollection(collectionName);
    ...
    public Document findOne(String dbName, String collectionName, Bson filter) {
        final List<Document> resultList = new ArrayList<Document>();
        if (filter != null) {
            FindIterable<Document> docs = mongoClient.getDatabase(dbName).getCollection(collectionName).find(filter).limit(1);
            docs.forEach(new Block<Document>() {
